...
import { getUpcomingDeadlines, getUpcomingTasksByDate } from '@/models/taskModel';
...
export function DeadlineAlertsProvider({ children }) {
  ...
  const load = useCallback(async () => {
    ...
    const t = todayStr();
    try {
      const [personal, calendar, team] = await Promise.all([
        getUpcomingDeadlines(user.id, addDays(t, -30), addDays(t, 7)).catch(() => []),
        getUpcomingTasksByDate(user.id, addDays(t, -7), addDays(t, 7)).catch(() => []),
        getUpcomingTeamDeadlines(addDays(t, -30), addDays(t, 7)).catch(() => []),
      ]);
      const all = [...personal, ...calendar, ...team];
      setOverdue(all.filter(x => x.deadline < t));
      setDueToday(all.filter(x => x.deadline === t));
      setSoon(all.filter(x => x.deadline > t));
    } catch (e) {
      console.error('[DeadlineAlerts] load 실패:', e.message);
    }
  }, [user]);
