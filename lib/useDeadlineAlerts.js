...
export function DeadlineAlertsProvider({ children }) {
  ...
  const load = useCallback(async () => {
    if (!user) return;
    const t     = todayStr();
    const tasks = await getUpcomingDeadlines(user.id, addDays(t, -30), addDays(t, 7));
    setOverdue(tasks.filter(x => x.deadline < t));
    setDueToday(tasks.filter(x => x.deadline === t));
    setSoon(tasks.filter(x => x.deadline > t));
  }, [user]);
